/*
  # Comprehensive Database Audit and Leaderboard Enhancement

  1. Database Connectivity Verification
    - All existing tables are functional and properly connected
    - Current schema includes: users, events, bets, transactions, bet_options, payment_methods
    - All foreign key relationships are properly established

  2. Leaderboard System Enhancement
    - Add computed columns for leaderboard metrics
  ...
  3. Streak Tracking System
    ...
    - Create streak calculation functions
    - Implement streak reset logic
    - Add streak reward multipliers
...
-- Enhanced user policies for new columns
CREATE POLICY "Admin can read any user"
  ON users
  FOR SELECT
  TO public
  USING (true);

CREATE POLICY "Admin can update user balance"
  ON users
  FOR UPDATE
  TO authenticated
  USING (
