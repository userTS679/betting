...
interface MobileOptimizedCardProps {
  ...
  className?: string;
  variant?: 'default' | 'compact' | 'minimal' | 'event';
  interactive?: boolean;
  loading?: boolean;
}
...
export const MobileOptimizedCard: React.FC<MobileOptimizedCardProps> = ({
  ...
  const baseClasses = `
    ...
    ${interactive ? 'cursor-pointer hover:shadow-lg active:scale-[0.98] touch-manipulation' : ''}
    ${loading ? 'animate-pulse' : ''}
  `;

  const variantClasses = {
    default: 'rounded-xl p-4 shadow-md hover:shadow-lg',
    compact: 'rounded-lg p-3 shadow-sm hover:shadow-md',
    minimal: 'rounded-lg p-2 shadow-sm',
    event: 'rounded-2xl shadow-lg hover:shadow-xl overflow-hidden'
  };
    ...
    return (
      <div className={`${baseClasses} ${variantClasses[variant]} ${responsiveClasses} ${className}`}>
        <div className="animate-pulse">
          ...
          <div className="h-8 bg-slate-200 dark:bg-slate-700 rounded mb-3"></div>
...
export const OptimizedEventCard: React.FC<OptimizedEventCardProps> = ({
  ...
  return (
    <MobileOptimizedCard variant="event" interactive>
      {/* Header with Category and Timer */}
      <div className={`bg-gradient-to-r ${getCategoryGradient(event.category)} p-3 sm:p-4 text-white -m-4 mb-4`}>
      ...
      {/* Pool Information */}
      <div className="bg-gradient-to-r from-emerald-50 to-blue-50 dark:from-emerald-900/20 dark:to-blue-900/20 p-3 rounded-lg mb-4">
